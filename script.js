...
function submitFeelingsEntry() {
    ...
    fetch(scriptURL, { method: 'POST', body: formData })
        ...
        .then(data => {
            if (data.status === 'error') throw new Error(data.message || 'Unknown error from server submitting feelings.');
...
async function fetchAndDisplayFeelingsEntries() {
    ...
    if (!feelingsListContainer) { console.error('"feelingsEntriesList" not found.'); navigateToFeelingsPage('feelingsPage1'); return; }
    ...
    try {
        ...
        if (feelingsListContainer) feelingsListContainer.innerHTML = `<p>Error loading entries. Check console.</p>`;
...
async function fetchDiaryEntries() {
    ...
    try {
        ...
        } else console.error('Error fetching diary entries from server:', data.message);
    } catch (error) { console.error('Failed to fetch diary entries (network/fetch error):', error); }
}

function renderCalendar(date) {
    const calendarGrid = document.getElementById('calendarGrid');
    const monthYearDisplay = document.getElementById('currentMonthYear');
    if (!calendarGrid || !monthYearDisplay) { console.error("Calendar elements not found."); return; }
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        // *** CRUCIAL FIX FOR DIARY DATE: Use local components for YYYY-MM-DD ***
        ...
        dayCell.addEventListener('click', () => {
            ...
            if (diaryEntries[dayCell.dataset.date]) viewDiaryEntry(dayCell.dataset.date);
            else openDiaryEntry(dayCell.dataset.date);
        });
        calendarGrid.appendChild(dayCell);
    }
}

function openDiaryEntry(dateString) { // dateString is YYYY-MM-DD from local components
    document.getElementById('selectedDate').value = dateString;
    console.log('openDiaryEntry received dateString:', dateString);

    const dateParts = dateString.split('-');
    if (dateParts.length !== 3) { alert('Invalid date: ' + dateString); return; }
    const yearNum = parseInt(dateParts[0], 10);
    const monthNum = parseInt(dateParts[1], 10) - 1; // JS months 0-11
    const dayNum = parseInt(dateParts[2], 10);
    const dateObj = new Date(yearNum, monthNum, dayNum); // Creates date in local timezone

    if (isNaN(dateObj.getTime())) { alert('Invalid date created: ' + dateString); return; }
...
function viewDiaryEntry(dateString) {
    ...
    if (!entry) { alert('No entry for ' + dateString); return; }

    const dateParts = dateString.split('-');
    if (dateParts.length !== 3) { alert('Invalid date for view: ' + dateString); return; }
...
function submitDiaryEntry() {
    ...
    if (!thoughts) { alert('Please write some thoughts.'); return; }
    ...
    fetch(scriptURL, { method: 'POST', body: formData })
        ...
        .then(data => {
            if (data.status === 'error') throw new Error(data.message || 'Error saving diary from server.');
        ...
        if (serverData.status === 'success' && serverData.data && serverData.data.length > 0) {
            ...
            sortedEntries.forEach(entry => {
                ...
                if (dateParts.length === 3) {
                    ...
                    if (!isNaN(entryDateObj.getTime())) formattedDate = entryDateObj.toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
                }
