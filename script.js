...
function initCountdown() {
    ...
    const saved = localStorage.getItem('hetuAnniversary');
    if (!saved) {
        // No date set — show a tap-to-configure message
        document.getElementById('cdSince').textContent = 'Tap ⚙️ below to set your date';
    ...
    const { date: startStr, label } = JSON.parse(saved);
    const start = new Date(startStr + 'T00:00:00');

    document.getElementById('cdSince').textContent = label
    ...
    function tick() {
        ...
        if (!daysEl) { clearInterval(countdownInterval); return; }
...
function openAnniversaryModal() {
    const saved = localStorage.getItem('hetuAnniversary');
    if (saved) {
        const { date, label } = JSON.parse(saved);
        document.getElementById('anniversaryDate').value  = date;
        document.getElementById('anniversaryLabel').value = label || '';
    }
...
function saveAnniversary() {
    ...
    if (!date) { showCustomPopup('Missing date', 'Please pick your start date 💕'); return; }
    localStorage.setItem('hetuAnniversary', JSON.stringify({ date, label }));
    closeAnniversaryModal();
    initCountdown();
    showCustomPopup('Saved! 💖', 'Your anniversary date has been set.');
}


// ═══════════════════════════════════════════════════════════════
//  LOVE NOTES (local, lightweight)
// ═══════════════════════════════════════════════════════════════
...
function loadLoveNotes() {
    ...
    recent.forEach((note, idx) => {
        ...
        const avatar = note.author === 'Prath' ? '🐼' : '🐰';
        const cls    = (note.author || '').toLowerCase();
        const time   = formatRelativeTime(new Date(note.ts));

        item.innerHTML = `
            <div class="note-avatar ${cls}">${avatar}</div>
            <div class="note-body">
                <div class="note-text">${escapeHtml(note.text)}</div>
                <div class="note-meta">${note.author || 'Unknown'} · ${time}</div>
            </div>
            <button class="note-del" onclick="deleteLoveNote(${origIdx})" title="Delete">✕</button>
        `;
...
function saveLoveNote() {
    ...
    if (!text) { showCustomPopup('Empty note!', 'Write something first 💕'); return; }
    ...
    showCustomPopup('Sent! 💌', 'Your love note has been left for them.');
}
...
/** Simple relative time: "just now", "2h ago", etc. */
...
function checkLoginStatus() {
    ...
    // Init home widgets after login check
    if (currentUser) {
        setTimeout(() => { initCountdown(); loadLoveNotes(); showMusicPill(); }, 50);
    }
}
...
/**
 * Determines which phase of the cycle the user is in and shows tips.
 */
function renderCyclePhase() {
    const panel = document.getElementById('cyclePhasePanel');
    if (!panel) return;
    ...
    const today       = new Date(); today.setHours(0,0,0,0);
    const dayOfCycle  = Math.floor((today - lastStart) / 86400000) + 1; // day 1 = period start

    // Phase boundaries (typical for 28-day cycle, scaled by actual cycle length)
    const ratio = cycleLength / 28;
    const menstrualEnd  = Math.round(5 * ratio);
    const follicularEnd = Math.round(13 * ratio);
    const ovulationEnd  = Math.round(14 * ratio);
    // luteal = ovulationEnd+1 → cycleLength

    // Update day labels
    document.getElementById('phaseMenstrualDays').textContent  = `1–${menstrualEnd}`;
    document.getElementById('phaseFollicularDays').textContent = `${menstrualEnd+1}–${follicularEnd}`;
    document.getElementById('phaseOvulationDays').textContent  = `~${follicularEnd+1}`;
    document.getElementById('phaseLutealDays').textContent     = `${ovulationEnd+1}–${cycleLength}`;

    // Remove active from all
    ['phaseMenstrual','phaseFollicular','phaseOvulation','phaseLuteal'].forEach(id => {
        document.getElementById(id)?.classList.remove('active-phase');
    });

    let currentPhaseName = '', currentPhaseEl = '', tip = '';

    if (dayOfCycle <= 0 || dayOfCycle > cycleLength + 7) {
        // Outside expected window
        document.getElementById('phaseYouAre').textContent = 'Update your period data to see your phase';
        document.getElementById('phaseTip').textContent    = '';
        return;
    } else if (dayOfCycle <= menstrualEnd) {
        currentPhaseEl  = 'phaseMenstrual';
        currentPhaseName = '🌺 Menstrual phase';
        tip = 'Rest is important now. Gentle movement, warm drinks, and self-care go a long way. Prath, be extra gentle and understanding today 💕';
    } else if (dayOfCycle <= follicularEnd) {
        currentPhaseEl  = 'phaseFollicular';
        currentPhaseName = '🌱 Follicular phase';
        tip = 'Energy is building! Great time for new plans, creativity, and social activities. Enjoy the rising mood and motivation.';
    } else if (dayOfCycle <= ovulationEnd) {
        currentPhaseEl  = 'phaseOvulation';
        currentPhaseName = '✨ Ovulation phase';
        tip = 'Peak energy and confidence! Communication feels easy, and connection with others is at its highest. Make the most of it 🌟';
    } else if (dayOfCycle <= cycleLength) {
        currentPhaseEl  = 'phaseLuteal';
        currentPhaseName = '🌙 Luteal phase';
        tip = 'Winding down toward the next cycle. Some days may feel heavier — that\'s completely normal. Cozy time, journaling, and rest are your best friends now.';
    } else {
        currentPhaseEl  = 'phaseMenstrual';
        currentPhaseName = '🔔 Period may be arriving soon';
        tip = 'Your next period is approaching. Prep with heating pads, comfort snacks, and lots of love 💕';
    }

    document.getElementById(currentPhaseEl)?.classList.add('active-phase');
    document.getElementById('phaseYouAre').textContent = `Day ${Math.max(1, dayOfCycle)} of ${cycleLength} · ${currentPhaseName}`;
    document.getElementById('phaseTip').textContent    = tip;
}
...
function renderPeriodHistory() {
    ...
    if (strip && periodData.length > 0) {
        ...
        // Average duration
        const withEnd = periodData.filter(e => e.endDate && e.endDate !== e.startDate);
        if (withEnd.length > 0) {
            const avgDur = withEnd.reduce((sum, e) => {
                const d = Math.round((new Date(e.endDate + 'T00:00:00') - new Date(e.startDate + 'T00:00:00')) / 86400000) + 1;
                return sum + d;
            }, 0) / withEnd.length;
            document.getElementById('statAvgDuration').textContent = Math.round(avgDur) + 'd';
        }

        // Average cycle between consecutive periods
        const sorted2 = [...periodData].sort((a,b) => new Date(a.startDate) - new Date(b.startDate));
        if (sorted2.length > 1) {
            const gaps = [];
            for (let i = 1; i < sorted2.length; i++) {
                const gap = Math.round((new Date(sorted2[i].startDate + 'T00:00:00') - new Date(sorted2[i-1].startDate + 'T00:00:00')) / 86400000);
                if (gap > 10 && gap < 60) gaps.push(gap);
            }
            if (gaps.length > 0) {
                const avg = Math.round(gaps.reduce((a,b) => a+b, 0) / gaps.length);
                document.getElementById('statAvgCycle').textContent = avg + 'd';
            }
        }
    } else if (strip) {
...
// ═══════════════════════════════════════════════════════════════
//  CYCLE WHEEL — Canvas rendering
// ═══════════════════════════════════════════════════════════════

const WHEEL_PHASES = [
    { id:'m', name:'Period',     emoji:'🌺', cls:'pb-m',
      tip:'Rest is everything right now. Warmth, gentle care, and Prath\'s patience makes all the difference 💕' },
    { id:'f', name:'Follicular', emoji:'🌱', cls:'pb-f',
      tip:'Energy is building! Great time for new plans, creativity, and social fun. Mood lifts naturally 🌿' },
    { id:'o', name:'Ovulation',  emoji:'✨', cls:'pb-o',
      tip:'Peak energy and confidence! Connection and communication feel effortless today 🌟' },
    { id:'l', name:'Luteal',     emoji:'🌙', cls:'pb-l',
      tip:'Winding down toward the next cycle. Some days feel heavier — that\'s completely normal. Journaling and cozy time help 🌙' },
];
const WHEEL_COLORS  = ['#ffb3b3','#a5d6a7','#fff176','#ce93d8'];
const WHEEL_BORDERS = ['#e57373','#66bb6a','#fdd835','#ab47bc'];

function buildPhaseBadges() {
    const container = document.getElementById('phaseBadges');
    if (!container) return;
    container.innerHTML = '';
    WHEEL_PHASES.forEach(p => {
        const b = document.createElement('div');
        b.className = 'phase-badge ' + p.cls;
        b.id = 'pb-' + p.id;
        b.innerHTML = `<span class="pb-emoji">${p.emoji}</span><span class="pb-name">${p.name}</span><span class="pb-days"></span>`;
        container.appendChild(b);
    });
}

function drawCycleWheel(dayOfCycle, cycleLength) {
    ...
    const activePhase = getPhaseFromDay(dayOfCycle, cycleLength);

    const segStart = -Math.PI / 2; // top
    let angle = segStart;
    const totalSegs = WHEEL_PHASES.length;

    WHEEL_PHASES.forEach((phase, i) => {
        const segDays = bounds[i] - (i === 0 ? 0 : bounds[i-1]);
        const segAngle = (segDays / cycleLength) * Math.PI * 2;
        const isActive = i === activePhase;
        const outerR = isActive ? R + 8 : R;
        const innerR = isActive ? rIn - 5 : rIn;

        // Arc segment
        ctx.beginPath();
        ctx.arc(cx, cy, outerR, angle, angle + segAngle - 0.03);
        ctx.arc(cx, cy, innerR, angle + segAngle - 0.03, angle, true);
        ctx.closePath();
        ctx.fillStyle = WHEEL_COLORS[i];
        ctx.fill();
        ctx.strokeStyle = isActive ? WHEEL_BORDERS[i] : 'rgba(255,255,255,0.6)';
        ctx.lineWidth = isActive ? 2.5 : 1.5;
        ctx.stroke();

        // Emoji in middle of segment
        const midA = angle + segAngle / 2;
        const emoR = (outerR + innerR) / 2;
        ctx.font = `${isActive ? 20 : 17}px Arial`;
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText(phase.emoji, cx + Math.cos(midA) * emoR, cy + Math.sin(midA) * emoR);

        // Update badge day ranges
        const start = i === 0 ? 1 : bounds[i-1] + 1;
        const end   = bounds[i];
        const badge = document.getElementById('pb-' + phase.id);
        if (badge) {
            badge.querySelector('.pb-days').textContent = start === end ? `Day ${start}` : `Days ${start}–${end}`;
            badge.classList.toggle('active', i === activePhase);
        }
    ...
    // Day marker
    const dayFrac = (dayOfCycle - 1) / cycleLength;
    const markerAngle = segStart + dayFrac * Math.PI * 2;
    const markerR = R + 16;
    ...
    ctx.beginPath();
    ctx.moveTo(cx + Math.cos(markerAngle) * (rIn - 2), cy + Math.sin(markerAngle) * (rIn - 2));
    ctx.lineTo(cx + Math.cos(markerAngle) * (R + 3),   cy + Math.sin(markerAngle) * (R + 3));
    ctx.strokeStyle = '#e85d8a';
    ctx.lineWidth = 2.5;
    ctx.stroke();

    // Marker circle with day number
    ctx.beginPath();
    ...
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText('Day ' + dayOfCycle, cx, cy - 9);
    ctx.fillStyle = '#9e6d80';
    ctx.font = '11px Nunito,sans-serif';
    ctx.fillText('of ' + cycleLength, cx, cy + 9);
}

function getPhaseFromDay(day, cycleLength) {
...
/** Full wheel render — replaces old renderCyclePhase */
function renderCyclePhase() {
    const panel = document.getElementById('cyclePhasePanel');
    if (!panel) return;
    ...
    const dayOfCycle  = Math.max(1, Math.min(raw, cycleLength + 5));

    drawCycleWheel(dayOfCycle, cycleLength);

    const pi  = getPhaseFromDay(dayOfCycle, cycleLength);
    const ph  = WHEEL_PHASES[pi];

    const tipLabel = document.getElementById('phaseTipLabel');
    const tipText  = document.getElementById('phaseTipText');
    if (tipLabel) tipLabel.textContent = ph.emoji + ' ' + ph.name + ' phase · Day ' + dayOfCycle + ' of ' + cycleLength;
    if (tipText)  tipText.textContent  = ph.tip;
}


// ═══════════════════════════════════════════════════════════════
//  STORY / SCROLL-DRIVEN MEMORY LANE
// ═══════════════════════════════════════════════════════════════

let storyCurrent = 0;
const STORY_GRADIENTS = [
    'radial-gradient(ellipse at 25% 50%,#3d1025 0%,#0d0608 68%)',
    'radial-gradient(ellipse at 75% 40%,#0d2510 0%,#0d0608 68%)',
    'radial-gradient(ellipse at 50% 60%,#2a2000 0%,#0d0608 68%)',
    'radial-gradient(ellipse at 40% 30%,#0a1535 0%,#0d0608 68%)',
    'radial-gradient(ellipse at 65% 55%,#1a0a30 0%,#0d0608 68%)',
    'radial-gradient(ellipse at 20% 35%,#1a1500 0%,#0d0608 68%)',
];

function renderTimeline() {
    const stage      = document.getElementById('storyStage');
    const strip      = document.getElementById('timelineContainer');
    const dotsEl     = document.getElementById('storyDots');
    const counterEl  = document.getElementById('storyCounter');
    const bgGrad     = document.getElementById('storyBgGrad');
    if (!stage || !strip) return;

    // Clear slides (keep nav, bg, counter)
    stage.querySelectorAll('.story-slide').forEach(s => s.remove());
    strip.innerHTML = '';
    ...
    if (sorted.length === 0) {
        stage.innerHTML += '<p style="color:rgba(255,255,255,.5);text-align:center;padding:40px;">No memories yet — add your first one! 📸</p>';
        return;
    }
    ...
    sorted.forEach((item, idx) => {
        ...
        slide.className = 'story-slide' + (idx === 0 ? ' s-active' : '');
        slide.dataset.idx = idx;

        const polaroid = document.createElement('div');
        polaroid.className = 'story-polaroid';
        polaroid.style.setProperty('--rot', rot + 'deg');
        polaroid.onclick = () => openMemoryModal(item, origIdx);

        const img = document.createElement('img');
        img.className = 'story-polaroid-img';
        img.src = item.img; img.alt = item.title;
        img.onerror = () => {
            img.remove();
            const fallback = document.createElement('div');
            fallback.className = 'story-polaroid-img broken';
            fallback.textContent = '📷';
            polaroid.prepend(fallback);
        };

        const caption = document.createElement('div');
        caption.className = 'story-caption';
        caption.innerHTML = `<span class="story-caption-date">${dateStr}</span><span class="story-caption-title">${item.title}</span>`;
        ...
        const chapterNames = ['Chapter One','A Beautiful Day','Golden Moments','Our Journey','Forever Together','Love & Laughter','Sweet Memories','New Adventures'];
        textBlock.innerHTML = `
            <div class="story-chapter">${chapterNames[idx % chapterNames.length]}</div>
            <div class="story-desc">${item.desc || 'A moment to remember forever.'}</div>
        `;
        ...
        thumb.onclick = () => goToStorySlide(idx);

        const tImg = document.createElement('img');
        tImg.className = 'thumb-img'; tImg.src = item.img; tImg.alt = item.title;
        tImg.onerror = () => {
            tImg.remove();
            const fb = document.createElement('div');
            fb.className = 'thumb-img broken'; fb.textContent = '📷';
            thumb.prepend(fb);
        };

        const tTitle = document.createElement('div');
        tTitle.className = 'thumb-title'; tTitle.textContent = item.title;
        ...
        if (dotsEl) {
            ...
            dot.className = 'story-dot' + (idx === 0 ? ' on' : '');
            dot.onclick = () => goToStorySlide(idx);
            dotsEl.appendChild(dot);
        }
    });

    // Counter
    if (counterEl) counterEl.textContent = `1 / ${sorted.length}`;

    // Prev / Next buttons
    const prevBtn = document.getElementById('storyPrev');
    const nextBtn = document.getElementById('storyNext');
    if (prevBtn) prevBtn.onclick = () => goToStorySlide(storyCurrent - 1);
    if (nextBtn) nextBtn.onclick = () => goToStorySlide(storyCurrent + 1);
    ...
    stage.addEventListener('touchend',   e => {
        ...
        if (Math.abs(dx) > 45) goToStorySlide(storyCurrent + (dx > 0 ? 1 : -1));
    }, { passive: true });
    stage.addEventListener('mousedown', e => { swipeStart = e.clientX; });
    stage.addEventListener('mouseup',   e => {
        const dx = swipeStart - e.clientX;
        if (Math.abs(dx) > 45) goToStorySlide(storyCurrent + (dx > 0 ? 1 : -1));
    });

    // Init gradient
    if (bgGrad) bgGrad.style.background = STORY_GRADIENTS[0];
}

function goToStorySlide(n) {
    ...
    // Animate out current
    slides[storyCurrent].classList.remove('s-active');
    slides[storyCurrent].classList.add('s-exit');
    setTimeout(() => slides[storyCurrent]?.classList.remove('s-exit'), 700);

    // Thumbs
    thumbs[storyCurrent]?.classList.remove('active');
    dots[storyCurrent]?.classList.remove('on');

    storyCurrent = n;

    // Animate in
    slides[storyCurrent].classList.add('s-active');
    thumbs[storyCurrent]?.classList.add('active');
    dots[storyCurrent]?.classList.add('on');

    // Scroll thumb into view
    thumbs[storyCurrent]?.scrollIntoView({ behavior:'smooth', block:'nearest', inline:'center' });

    // Update gradient & counter
    if (bgGrad) bgGrad.style.background = STORY_GRADIENTS[storyCurrent % STORY_GRADIENTS.length];
    if (counter) counter.textContent = `${storyCurrent + 1} / ${total}`;
}
...
const musicPlaylist = [
    ...
    { title: 'Tera Ban Jaunga',artist: 'Akhil Sachdeva', emoji: '💖' },
...
function toggleMusicPlayer() {
    ...
    if (musicPlayerOpen) renderMusicTrack();
}
...
function renderMusicTrack() {
    ...
    if (coverEl)  coverEl.textContent  = track.emoji;
}
...
function musicToggle() {
    ...
    if (coverEl) coverEl.classList.toggle('spinning', musicPlaying);
...
function musicNext() {
    ...
    if (musicPlaying) musicToggle(); // restart
    if (!musicPlaying) { musicPlaying = false; musicToggle(); }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (bar) bar.addEventListener('click', (e) => {
        ...
        musicProgress = ((e.clientX - rect.left) / rect.width) * 100;
        const fill = document.getElementById('musicProgressFill');
        if (fill) fill.style.width = musicProgress + '%';
    });
});
