...
function addPeriodEntry() {
    ...
    const entry={ startDate:start, endDate:end, mood:selectedMood, loggedBy:currentUser, timestamp:new Date().toISOString() };
    ...
    // Sync to Google Sheets
    const fd=new FormData();
    fd.append('formType','periodEntry');
    fd.append('action','add');
    fd.append('date',start);
...
function deleteLogEntry(idx) {
    showCustomPopup('Delete this log?','Remove this period entry?',null,confirmed=>{
        ...
        if(entry){
            ...
            fd.append('date',entry.startDate);
            fd.append('submittedBy',entry.loggedBy||currentUser);
            fetch(scriptURL,{method:'POST',body:fd,mode:'cors'}).catch(()=>{});
        }
    });
}
