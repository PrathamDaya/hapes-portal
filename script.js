// --- FULLY UPDATED script.js with User Login, Attribution, Reply, Dare Game, and Text Pressure Title ---
...
class TextPressure {
    ...
    init() {
        ...
        this.titleRef.className = 'text-pressure-title';
        ...
        this.chars.forEach(char => {
            ...
            this.spansRef.push(span);
        });

        this.setSize();
        window.addEventListener('resize', this.setSize.bind(this));
        
        // Use container for mouse events to localize the effect
        this.container.addEventListener('mousemove', this.handleMouseMove.bind(this));
        this.container.addEventListener('touchmove', this.handleTouchMove.bind(this), { passive: true });
        this.container.addEventListener('mouseleave', this.handleMouseLeave.bind(this));

        // Set initial mouse position to the center of the container
        const rect = this.container.getBoundingClientRect();
        this.mouse.targetX = rect.left + rect.width / 2;
        this.mouse.targetY = rect.top + rect.height / 2;
    ...
    setSize() {
        ...
        let newFontSize = containerW / (this.chars.length / 2.2);
        newFontSize = Math.max(newFontSize, this.options.minFontSize);
        this.titleRef.style.fontSize = `${newFontSize}px`;
    }
    ...
    animate() {
        ...
        this.spansRef.forEach(span => {
            ...
            const wght = Math.floor(getAttr(d, 200, 900));
            const wdth = Math.floor(getAttr(d, 80, 150));
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize Text Pressure Title
    const textPressureContainer = document.getElementById('text-pressure-container');
    if(textPressureContainer) {
        new TextPressure(textPressureContainer, 'For my Hetu', { minFontSize: 36 });
    }
    ...
    if (nextMonthBtn) {
     ...
     console.log('DOM loaded. All functions initialized.');
});
