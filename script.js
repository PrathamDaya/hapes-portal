...
function initCountdown() {
    // Use saved date or fall back to default
    let saved = null;
    try { saved = JSON.parse(localStorage.getItem('hetuAnniversary')); } catch(e) {}

    const dateStr = saved?.date  || DEFAULT_ANNIVERSARY;
...
async function loadPlaylist() {
    ...
    if(musicPlaylist.length===0){
        ...
        document.getElementById('musicArtist').textContent='Add .mp3 files to assets/music/ and list them in playlist.json';
        return;
    }
    musicIdx=0;
    loadTrack(musicIdx);
